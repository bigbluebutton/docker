...
RUN echo "deb [arch=amd64,arm64] https://repo.mongodb.org/apt/ubuntu xenial/mongodb-org/3.4 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-3.4.list
...
RUN apt-get install -y kurento-media-server mongodb-org nginx ffmpeg libreoffice tidy yq curl tidy h264-gst-plugins-bad-1.5 xmlstarlet git-core build-essential vorbis-tools sox mencoder ruby-dev libxslt1-dev libxml2-dev libncurses5-dev gnuplot zlib1g-dev python3 ruby rsync libsystemd-dev poppler-utils ttf-mscorefonts-installer ghostscript imagemagick psmisc 
...
# -- Final BigBlueButton dependcies
#RUN apt-get install -y libcurl4-openssl-dev libldns2 liblua5.2-0 libopusenc0 libopusfile0 libspeexdsp1 netcat-openbsd python3-bs4 python3-html5lib python3-icu python3-lxml python3-webencodings
