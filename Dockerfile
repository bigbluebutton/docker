...
RUN apt-get update && apt-get install -y rubygems && gem install bundler -v 1.16.1
...
# -- Mongo configuration
RUN echo 'replication.replSetName: rs0' >> /etc/mongod.conf && sed -i 's/127.0.0.1/127.0.1.1/' /etc/mongod.conf
